...
// Convert UYVY to RGB888
static void uyvy_to_rgb888(uint8_t *uyvy, uint8_t *rgb, int width, int height) {
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            int y_index = (j * width + i) * 2;
            int u_index = y_index + 1;
            int v_index = y_index + 3;

            uint8_t y = uyvy[y_index];
            uint8_t u = uyvy[u_index] - 128;
            uint8_t v = uyvy[v_index] - 128;
...
void camera_lvgl_init(const char *dev_path) {
    ...
    fmt.fmt.pix_mp.pixelformat = V4L2_PIX_FMT_UYVY;  // Chuyển từ NV12 sang UYVY
...
static void camera_lvgl_display_once(lv_obj_t *img) {
    ...
    uint8_t *uyvy = buffers[buf.index].start;

    static uint8_t rgb_full[FRAME_WIDTH * FRAME_HEIGHT * 3];
    uyvy_to_rgb888(uyvy, rgb_full, FRAME_WIDTH, FRAME_HEIGHT);
